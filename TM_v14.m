...
loc_param = '/home/klaferri/Desktop/Research/ThermalModel/Trough2/Slice6/Lag_'+string(thickness)+'mm/';
paramFile = loc_param + 'paramFile_slope.mat';
fprintf('file location %s', paramFile)
in = load(paramFile);
s = in.C;
...
direc = string(loc_param + 'Output')
if exist(string(direc), 'dir') == 0
    mkdir(string(direc));
end
cd(direc)
...
strFileLocationParam = strcat(direc+'/', datestr(now, 'yyyy-mm-dd'), '/', datestr(now, 'yyyy-mm-dd_HHMM'));
...
for L04Step = s.Laskar_begin:s.Laskar_step:s.Laskar_end
    ...
    [TotalFree, TotalForced, TotalSublimation] = sublCalculator(L04Step, s, Ls0, L04obl, atmosPressSF12, SLOPEDaverageDiurnalSurfTemps, REGminDiurnalSurfTemps, REGdiurnalTsurfCurves, dt, densityIce, avogadro, s.windSpeed); 
    sublCalculator2;
